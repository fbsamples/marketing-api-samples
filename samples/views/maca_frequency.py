...
from samples.samplecode import maca_frequency
...
EVENT_CHOICES = (
    ('fb_mobile_activate_app', 'App Launches'),
...
PERIOD_HELP = 'The time period you want to calculate the event frequency'
...
class MacaFrequencyView(SampleBaseView):

    BUTTON_TEXT = "Create Mobile App Custom Audience"

    @fbads_sample('samples.samplecode.maca_frequency')
    def get(self, request, *args, **kwargs):
        form = MacaFrequencyForm(request.last_error_post)
        return self.render_form(request, form)

    @fbads_sample('samples.samplecode.maca_frequency')
    def post(self, request, *args, **kwargs):
        form = MacaFrequencyForm(request.POST, request.FILES)
        ...
        sample = maca_frequency.MacaFrequencySample()
